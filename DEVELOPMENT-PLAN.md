...
| F38 | **From Method to Meaning v2** — mountain-journey UI, avatar climber, typed or live-voice reasoning (supersedes F20b) | Could | XL+ | F33, F36; voice half needs D18/D19 | 4 | ◐ **F38-lite SHIPPED 2026-09-02, NOT browser-verified** — typed-response path only; full-screen mountain view, avatar climbs on stage completion, all 12 questions/grades unchanged, stage-completion feedback, mastery summit with review. Genuinely Supabase-free: progress lives in `progress.steps.mm` (existing jsonb, no migration) so this actually works today despite the account-access block. Old printable modal kept, reachable via a "🖨 Printable version" link. F38-full (live voice tutor) still not started — blocked on D18/D19 as before. |
