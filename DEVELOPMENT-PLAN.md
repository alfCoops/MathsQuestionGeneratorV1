...
| F38 | **From Method to Meaning v2** — mountain-journey UI, avatar climber, typed or live-voice reasoning (supersedes F20b) | Could | XL+ | F33, F36; voice half needs D18/D19 | 4 | ◐ **F38-lite v2 SHIPPED 2026-09-02, NOT browser-verified** — mountain view redesigned with an actual illustrated peak backdrop + per-stage accent colours (Justify orange, Challenge purple, per Ryan's design brief) and a duration chip, replacing the earlier thin-dashed-line version Ryan found didn't read as a mountain. Answer flow now calls a real AI grading endpoint (`mmGrade()`, reusing F32's `genPOST`/token-refresh plumbing) when one exists, with feedback shown inline and a 2-attempt "mark as attempted" escape valve — but that endpoint (`/v1/grade`) isn't built yet (separate generator-service repo; contract written in `GRADE-ENDPOINT-SPEC.md`), so today it still degrades to the original "any non-empty answer advances" behaviour, same as before. Lesson-overview page redesign Ryan also asked for is deferred — reference mockup didn't attach. F38-full (live voice tutor) still not started — blocked on D18/D19 as before. |
