...
| F19 | Quiz engine v2 (misconception distractors, variant pools, grade stepping) | Should | L (+authoring) | F11, **F32** | 4 | ◐ **Milestone 1 (Order of Operations, 3 objectives) CODE SHIPPED 2026-09-11, NOT yet live** — plan at `.claude/plans/jolly-sprouting-puzzle.md`. New `OBJECTIVES` per-course content array (id+label+specRef, `MATHEMATICIANS`-pattern admin panel) — 3 seeded rows (`oo-1/2/3`), lesson 1a opted in via new `L.objectiveIds`. `questions_review` gains `objective_id`/`eligible_start` real columns; the F10b review queue (already rendering MCQ options/misconceptions/scaffold ladders since F32-x) now tags `kind:'question'` items with both. New sanitised `quiz_bank` view — unlike the static per-lesson quiz (answer key already sits in the page), a live cross-objective bank never ships `options[].correct`/misconception ahead of an answer; grading goes through a new `grade_quiz_answer()` RPC (SECURITY DEFINER) that logs `quiz_results`, upserts new `objective_mastery` (4-tier, D16) and `student_misconceptions` tables in one transaction — same trusted-server posture as `recompute_my_points()`. Client engine (`loadQuizBank`/`pickStartingQuestions`/`pickNextQuizQuestion`) is `pickGeneratedQuestion()`'s sibling per CLAUDE.md's own "swap point" note: climbs the same scaffold ladder on a wrong answer before falling back, steps difficulty up on correct. Wired into the real quiz UI via a new `openQuiz()` dispatch — a signed-in student on an `objectiveIds` lesson gets the new engine; everyone else (incl. every signed-out visitor, preserving D1's free 1a) keeps the untouched static quiz. Teacher dashboard gained a per-attempt Student/Topic/Objective/Result/Misconception/Support-needed table, and the existing class-wide misconception list was corrected to count distinct students (Ryan's own "M03 — 14 students" phrasing) rather than raw occurrences. Generate tab gained a Question-type + objective picker; `QUIZ-BATCH-ENDPOINT-SPEC.md` documents the `/v1/batch` extension the separate generator-service repo needs to actually produce this content — degrades safely (same as `/v1/import`/`/v1/grade`) until that's built. **Deliberately deferred, not silently dropped**: the energy-meter tie-in, weighted/Duolingo-style review-section scoring, and cross-topic persistent-misconception resurfacing from Ryan's fuller 31-07 spec — Milestone 1 is the prototype DEVELOPMENT-PLAN.md itself calls for before generalising. Also not resumable across a page reload yet (`progress.quiz_state` is reserved for this but a second sync path was cut from this pass). **Remaining before this is real for students**: run the new SQL in Supabase, the generator-service repo implementing `QUIZ-BATCH-ENDPOINT-SPEC.md`, and a teacher generating + approving ≥5 questions per objective. |
...
| F41 | **Public marketing pages** — Home, How It Works, Pricing (visual-only), About Us, sidebar-less nav+footer chrome | Could | L | — | 6 | ✅ **SHIPPED 2026-09-04, NOT browser-verified** — new `marketingChrome()`, a second sidebar-less page shell alongside `pageChrome()`; every existing app page (courses, lessons, admin, parent/teacher) is untouched. New routes `#/` (signed-out only — a signed-in visit to the bare URL still lands on `#/courses` unchanged), `#/how-it-works`, `#/pricing`, `#/about`. Pricing is **visual-only** — "Get Premium" shows a toast, no payment provider, F14 stays deferred until after the pilot. About Us copy is **placeholder**, mirroring themes already public on the live Wix site (qualified/DBS-checked tutor) — flagged for Ryan to review before it's real. **2026-09-04 follow-up:** Ryan confirmed the pages are live and asked for them to be Teacher-Editor-editable, starting with About Us. New `MARKETING` content object (same global "travels the existing save/publish path" treatment as `RESOURCES`, no new Supabase table) + a `marketingPanel()` in the Teacher Editor (reuses the shared rich-text toolbar/`rich()`/`richValue()` exactly like a lesson's Key Notes field) for the page heading + body. About Us also restyled to match the How It Works page's richer visual language (feature-icon row + a content card) instead of plain paragraphs, per Ryan's "mimic the style" note — Home/Pricing/How It Works get the same editable treatment as we work through each page next. **2026-09-04, second follow-up:** How It Works restyled again — the first pass was too plain vs. Ryan's mockup (inline emoji, no colour); added colour-coded icon circles per step, an illustrative progress-stepper demo, a "Research-Informed Learning" footer, and a new "Earn Points & Badges" section. That last one deliberately describes only the REAL, live points/badges system (`recompute_my_points()`, the `BADGES` array) — Ryan asked for an "energy bar that levels up through tiers" too, but that's F19 spec only (gated on pilot-complete, not started); flagged instead of quietly marketed as live, and Ryan chose to keep the copy honest rather than build F19 early or mark it "coming soon." |
| F42 | **Login/Signup redesign** — restyled auth card on the new F41 chrome, Parent/Student account-type selector, one-time welcome/avatar interstitial | Could | M | F41 | 6 | ✅ **SHIPPED 2026-09-04, NOT browser-verified** — `renderLogin()` moved onto `marketingChrome()`; `#/login?up=1` opens straight to Create account. Account-type selector reuses F13's existing `role` machinery (`ensureProfile`/`signUpEmail` gained an optional `role` param) — no migration needed, the "own profile" RLS policy already lets a user set their own role. A Parent signup routes straight to `#/parent` instead of needing a child's invite code first; the invite-code redeem flow is unchanged. New `#/welcome` — a one-time interstitial (nickname prompt if missing, then the avatar picker) gated by a new `prefs.onboarded` flag, so existing users are never shown it. **Scope trim**: the account-type selector only applies to email signup — an OAuth (F43) signup always creates a Student account today; converting to Parent afterwards uses the same existing invite-code flow. |
| F43 | **Social sign-in** — Google + Microsoft OAuth, email-only scope | Could | M + Ryan's manual OAuth app setup | F42 | 6 | ◐ **CODE SHIPPED 2026-09-04, NOT LIVE (needs Ryan's manual setup), NOT browser-verified** — `oauthSignIn()` calls `signInWithOAuth({provider, options:{scopes:'email', ...}})` for `google`/`azure`, requesting no name/photo (CLAUDE.md's data-minimisation rule for minors). The redirect return trip reuses the **existing** T3 `access_token=` guard/`renderAuthCallback()` unchanged. Found and fixed along the way: `onAuthStateChange` never called `onSignedIn()` for an OAuth-driven sign-in (only `signInEmail`/`signUpEmail` called it directly) — without the fix, an OAuth sign-in would leave `authProfile` null forever, no profile row ever created. New `OAUTH-SETUP.md` — Ryan needs to register a Google Cloud OAuth client and an Azure App registration and paste both into Supabase Auth → Providers before either button does anything; until then they show a clear "provider not enabled" error, not a dead click. |
| F44 | **One connected MasterMaths** — blue brand identity, How It Works reuses real lesson components, editable public content, parent-unlink, Energy Meter + ranks (integrated into the existing leaderboard, not a new one), Famous Mathematician avatars | Could | XL, phased | F41–F43 | 6 | ◐ **Phase 0 (blue identity) SHIPPED 2026-09-04.** Full 27-section brief from Ryan; planned in `/Users/ryanh7989/.claude/plans/jolly-sprouting-puzzle.md` with 6 phases and Ryan's sign-off on 3 architecture forks (Prior Knowledge stays outside the graded 4-step lesson flow everywhere — a deliberate existing design, not reversed; Energy is a new parallel server-recomputed value mirroring `recompute_my_points()`'s security model, not derived from points; mathematician avatars ship now with colour+initial placeholders, real art swapped in as Ryan generates it). **Phase 0** — `--blue`/`--blue-soft` were a legacy mislabel (identical hex to green); repointed to genuine blue (`#1d4ed8`/`#eff6ff`, new `--blue-mid:#3b82f6`), fixed dark/high-contrast theme overrides, explicitly pinned `.btn.solid`/`.btn.complete` to `--green` (primary actions stay green per the brief's own hierarchy), fixed one real clash found along the way (`.ad-steps` had a blue background with a hardcoded green border/text left over from the mislabel), nav-link hover and one Home section background now use the new blue tokens. Phases 1–6 (How It Works component reuse, editable content, parent unlink, Energy/ranks, mathematician avatars) not started — shipping one phase at a time per the plan's recommended order. **2026-09-04, same-day correction:** Ryan sent an actual "Meet Your Tutor" heading crop from the live site — Phase 0's vivid royal blue (`#1d4ed8`) wasn't the right tone. Decoded the PNG's raw pixels (not eyeballed) to get exact values: navy `#25395c`, sage-green `#55b685`. `--navy` now matches exactly; `--blue`/`--blue-mid`/`--blue-soft` are a lightened family off that *same* hue rather than an unrelated vivid blue. `--mm-green` (the brand/logo-green token) moved toward the sampled sage but darkened to `#1f7a52` — the raw sampled hex fails contrast where this token puts white text on a solid fill (`.ws-grade.g3`, `.unit-cta.strong`); kept the outgoing value's ~5:1 contrast ratio while shifting the hue. Also added `Baloo 2` (Google Font) as `--ff-head`, applied to the marketing nav brand and H1/H2 only — matching the reference image's bold rounded display style for brand moments, while body text/quiz/lesson content stays on Inter for readability. **Phase 1 (How It Works reuses real lesson components) SHIPPED 2026-09-04** — the 5-stage journey now renders the actual `.steps`/`.act-head`/`.pkc-card` markup and exact per-section copy from the real lesson page (index.html:967-1052) instead of bespoke marketing lookalikes; the old `.mkt-progress-demo`/`.mkt-step`/`.mkt-pkc-card` CSS (F41) was removed as dead code. Prior Knowledge stays outside the numbered steps here too, matching the real lesson's own documented design decision — not reversed. Buttons are omitted (static preview, not a functional lesson). **2026-09-04, icon redesign, same day:** Ryan sent a mockup of these exact cards (larger two-layer icons — pale halo + solid glyph circle — plus a small secondary badge, and a coloured sub-headline under each title) and confirmed he wants it applied to the real lesson page too, not just the marketing preview. Since How It Works reuses the real `.act-head`/`.ico` markup (Phase 1 above), this was one shared CSS/HTML change: `.ico` is now a two-layer badge (`--ic`/`--icbg` custom properties set per-instance, unchanged colours — blue/orange/purple/pink) with a `.ico2` secondary emoji badge (📄/📋/💬/📊) and a new bold coloured `.act-sub` line between the title and description. Buttons inside `.act-head` are now pill-shaped (`border-radius:999px`), matching the mockup — the existing `setBtn()` done/not-done text-and-colour logic (`.btn.done`) was untouched, so "Start" → "✓ Completed"/"✓ Passed" still works exactly as before, just pill-shaped now. Verified no JS reads `.ico`'s content as plain text (the new child `<span>`s are safe) and no `@media print` rule depends on the old flat-icon structure. **2026-09-04, dropdown add-on:** each How It Works card (including Prior Knowledge) gains a clickable chevron that expands a clear, detailed explanation of what that stage actually does — reuses the real `.body`/`.open>.body{display:block}`/chevron-rotate CSS already defined for the real lesson accordion, but with its own tiny toggle (`wireHiwCards()`) rather than the real `openAct()`/`toggleAct()`, which also touch real lesson state (`noteSection()`, video focus, one-at-a-time collapsing) meaningless on a static preview page. Scoped to the marketing page's own root element, so it can never attach to the real lesson's `#secPkc`/etc. **Cross-navigation fix, 2026-09-04:** once a signed-in student was inside the app (courses/lessons), there was no click path back to the public marketing pages at all — only a hand-typed URL. Added How It Works/Pricing/About Us links to the sidebar (same "open to everyone, not gated on sign-in" treatment as the existing F40 Resources link). **Home hero rebuild, same day:** Ryan asked for the homepage to match his mockup exactly — replaced the plain "one-to-one tutoring" text card with a layered app-preview composition (a mini dashboard card with a real SVG progress ring, a topic progress bar, and 4 coloured icon tiles, overlapped by a second card reusing the same `.steps`/`.node` progress-stepper already built for How It Works) plus a "For Secondary Students" pill above the heading and coloured circular backgrounds on the 4 feature icons. Built with a deterministic SVG `stroke-dasharray` ring rather than a `conic-gradient`, since it renders predictably without a live browser available to check it in. **Nav changes, same day:** renamed "For Students" to "Learn" and pointed it at `#/login` instead of straight to `#/courses` (a signed-out visitor now sees sign-in first; already-signed-in visitors are unaffected, `renderLogin()` already redirects them straight through). Then, per Ryan's follow-up, the bare-URL routing rule changed again: the marketing Home page now wins for **every** visitor at the empty hash, signed in or not (previously signed-in visitors skipped straight to `#/courses` to avoid disrupting bookmarks) — `renderMktNav()` already shows "My Courses" instead of "Log in" once signed in, so the one extra click to reach the dashboard is the accepted trade-off. **Phase 4 (parent/student unlink) SHIPPED 2026-09-04.** New `unlink_student(student_id)` SQL function (migrations.sql) — SECURITY DEFINER, scoped to `parent_user_id = auth.uid()` in its own WHERE clause exactly like `redeem_invite_code()` already is, so a parent can only ever remove their own link. Deliberately no separate RLS delete policy added — the RPC is the sole write path, matching the existing pattern, rather than opening a second (redundant) raw-client-delete route. `renderParent()`'s student cards gain a green "✓ Connected" chip and an Unlink button (confirms first, same `confirm()` pattern already used for rejecting a generated item); the empty state now explains what "linked" means and where a child finds their code, not just "no students yet." `get_or_create_invite_code()`/`redeem_invite_code()` themselves are untouched. **Phase 5 (Energy Meter + ranks) SHIPPED 2026-09-04.** Ryan refined the values mid-build to per-question granularity: every CORRECT quiz answer = 1 Energy, every Method to Meaning question answered (correct or not) = 1 Energy, Lesson Complete bonus = 10; Watch Lesson & Read Notes and the Question Generator deliberately contribute 0. Quiz answers were already recorded per-question (`quiz_results`); Method to Meaning had NO server-side per-question record at all before this — new `reasoning_answers` table (minimal columns, just enough to count) + a new `recordReasoningAnswer()` client call from the mountain-journey's `advance()`. `recompute_my_points()` extended (now returns `jsonb {points, energy_total}` instead of a bare int) to also compute and store `energy_total` — a separate, monotonic column from `points`, not derived from it, since Ryan's Energy weights don't match points' existing weights. Rank number/current-tier energy are pure derivations (`floor(energy_total/100)`, `energy_total%100`) computed client-side, never separately stored — "excess energy carries over" and "rank never loses energy" fall out of that for free. New sidebar Energy Meter widget (students only — a teacher/parent account never accrues Energy), a rank-up toast (compares against a localStorage-cached last-seen rank, primed on sign-in so a fresh browser's first activity can still detect a genuine rank-up), and the **existing** leaderboard (not a new one) now shows a rank chip alongside points. RANKS is a hardcoded 10-name array for this pass — admin-editable names/thresholds flagged as a clean follow-up, not built here. **Phase 6 (Famous Mathematician avatars) SHIPPED 2026-09-04 — the last phase.** F36's 10-emoji preset (`AVATARS`/`avatarGlyph()`) is fully replaced by a new `MATHEMATICIANS` content array (same global, admin-editable, array-of-objects pattern as `RESOURCES` — travels the existing save/publish path, no new Supabase table), seeded with 10 real mathematicians (Ada Lovelace, Alan Turing, Katherine Johnson, Ramanujan, Emmy Noether, Gauss, Euler, Hypatia, Newton, Mirzakhani), every `image` field deliberately blank since no character art exists yet — `avatarHtml()` renders a colour+initial placeholder wherever `image` is empty, so nothing shows a broken `<img>`; real art drops in later by editing one field per mathematician in the new Teacher Editor panel, no code change needed. `prefs.avatarId` now stores a mathematician id instead of an emoji — old emoji values and any deactivated/removed id both silently fall back to the first active mathematician (`avatarMathematician()`), so this resolves itself for existing users with no migration script. Propagated to every avatar call site: sidebar, mountain-summit, the F42 signup carousel (now shows name + description, not just a glyph), the settings picker (relabelled "Change Avatar"), and two genuinely new render sites that showed no avatar at all before this — the leaderboard (`energy_total`'s sibling column, `prefs->>'avatarId' as avatar_id`, added to the same view) and the parent dashboard. New `mathematiciansPanel()` (same array-editor pattern as `resourcesPanel()`) lets Ryan activate/deactivate/reorder (array position is the order, same convention as `WEEKS`/`RESOURCES` — no separate order field to keep in sync) and edit each entry's description; `id` itself isn't editable, since it's the value already stored in existing students' prefs. **Master brief (F44) complete** — all 6 phases shipped; remaining open items are Ryan's own follow-ups (regenerating images 2-4 in the boy-with-laptop's style, and eventually supplying real mathematician illustrations). **Phase 2+3 (editable content + tutorial video) SHIPPED 2026-09-04.** `MARKETING` extended (`MARKETING_DEFAULTS` constant, same back-compat merge pattern) with `homeHeading`/`homeSub`/`homeCta`, `howItWorksIntro`, `pricingIntro`, `tutorialVideoUrl` — all plain text except About Us's existing rich body, per the brief's "keep this simple, no unnecessary CMS." `marketingPanel()` gained matching fields (Home/How It Works/Pricing panels + a video panel reusing `storageUpload()`/`videoEmbedHtml()` — the exact same "file or embed link, friendly coming-soon placeholder" helper the real lesson video player already uses, not a new upload mechanism). Home's custom heading intentionally loses the default's 3-line colour-split treatment (renders as one plain line) — a deliberate simplicity trade-off, not a bug. The 5-stage journey and pricing plan cards themselves stay code-driven (only their intro lines are editable) so a content edit can't accidentally misrepresent the real lesson flow or break the pricing table structure. New "How to Use MasterMaths" video section added to How It Works, visually separated from the 5-stage journey per the brief. |
